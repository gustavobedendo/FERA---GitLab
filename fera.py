...
class MainWindow():    
    ...
    def __init__(self):
        ...
        self.style.configure("Treeview", font=global_settings.Font_tuple_Arial_10, rowheight=25, indent=10)
        ...
        self.exportinterval = utilities_export.ExportInterval(global_settings.pathpdfatual, global_settings.root)
                        ...
                        if(str(iidantigo!=str(iidnovo))):
                            ...
                            try:
                                ...
                                if(not self.treeviewObs.exists(str(iidnovo)+basepdf)):
                                    ...
                                    self.treeviewObs.insert(parent=str(iidnovo), iid=(str(iidnovo)+basepdf), text=ident+os.path.basename(basepdf), index=indexrelobs, tag=('relobs'))
...
"{{\\f2\\fswiss\\fprq2\\fcharset0 Arial;}}}}{{\\colortbl;\\red240\\green240\\blue240;\\red221\\green221\\blue221;\\red255\\green255\\blue255;\\red190\\green0\\blue0;}} {}}}")\
                ...
                try:
                    if(len(recordsx)>0):
                        ...
                        if(results != None and len(results) >0):  
                            ...
                            self.nhp.config(relief='raised', state='normal')
                            self.php.config(relief='raised', state='normal')                        
                            utilities_general.below_right(utilities_general.popup_window('<{}> - Nenhuma ocorrência encontrada!'.format(termo), False),\
    ...
    def printer(self):
        ...
        application_path = global_settings.get_application_path()
        try:
            if plt == "Linux":
    ...
    def createTopBar(self):      
        try:
            ...
            self.bselect = tkinter.Button(self.manipulationTool, image=global_settings.imselect, padx=20, command=self.activateSelection)
            ...
            self.baselect = tkinter.Button(self.manipulationTool, image=global_settings.imareaselect, padx=20, command=self.activateareaSelection)
            ...
            self.showbookmarks = tkinter.Button(self.manipulationTool, image=global_settings.showbookmarksi, padx=20, command=self.showAllBookmarks)
        ...
        for leaf in leaves:
            ...
            allitens.append((global_settings.searchResultsDict[leaf], global_settings.infoLaudo[pdf].mt, \
                             global_settings.infoLaudo[pdf].mb, global_settings.infoLaudo[pdf].me, global_settings.infoLaudo[pdf].md,\
                                 global_settings.infoLaudo[pdf].pixorgw, global_settings.infoLaudo[pdf].pixorgh))
    ...
    def addmarkerFromSearch(self, idobscat, event, first=True):
        ...
        if(len(children)>0 and first):
            ...
            if(pagina not in global_settings.infoLaudo[global_settings.pathpdfatual].quadspagina):
                if(first or pagina in global_settings.processed_pages):
                    global_settings.root.after(100, lambda: self.addmarkerFromSearch(idobscat, event, first=False))
            else:
                try:
                    sqliteconn =  utilities_general.connectDB(str(global_settings.pathdb), 5)
                    try:
                   ...
                   # def __init__(self, paginainit, paginafim, p0x, p0y, p1x, p1y, tipo, pathpdf, idobs, idobscat, obscat, conteudo, anottations):
                    ...
                    global_settings.root.after(1000, lambda: self.addmarkerFromSearch(idobscat, event, first=False))    
    ...
    def menuReports(self, event=None):
        try:
            ...
            self.reportbut.menu.add_separator()
            self.reportbut.menu.add_command(label='Sobre', image=global_settings.aboutimage , compound='left', command= lambda: utilities_general.popup_window(texto, False, imagepcp=self.tkphotologo))
            self.reportbut.pack()    
        except Exception as ex:
            utilities_general.printlogexception(ex=ex) 
        finally:
